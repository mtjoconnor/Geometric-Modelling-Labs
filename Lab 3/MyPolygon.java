...
public class MyPolygon extends Polygon {
    ...
    public void elevateOnce() {
      ...
      // Elevate by one degree in-place; iterate backwards to preserve old values.
      double lastX = tempX[np - 1];
      double lastY = tempY[np - 1];
      for (int i=np - 1; i>=1; i--) {
        double w1 = (double)(np - i) / (double)np;
        double w2 = (double)i / (double)np;
        tempX[i] = w1*tempX[i - 1] + w2*tempX[i];
        tempY[i] = w1*tempY[i - 1] + w2*tempY[i];
      }
      tempX[np] = lastX;
      tempY[np] = lastY;
      ...
      elevated = true;
      System.out.println("Degree elevated to " + (Elevated.npoints - 1));
    }
}
